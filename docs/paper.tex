...
Telex is conceptually similar to our work.
...
(for Telex a tag is a hash embedded in the TLS client randomness;
for us it is the HTTP Host header, hidden within an HTTPS stream).
An important difference is that Telex requires some degree of buy-in from network operators
whose incentives may not be aligned with circumventors'.
We require no active participation on the part of the frontend operator,
only a lack of active opposition.
