...
p <- p + geom_point() + xlab("Date") + ylab("Mean concurrent users") + scale_x_date(labels=date_format("%b %Y")) + scale_y_continuous(breaks=0:10, minor_breaks=NULL) + theme(axis.text=element_text(size=8), axis.title=element_text(size=8))
