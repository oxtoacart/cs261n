...
appears in Section~\ref{sec:analysis}.
...
Section~\ref{sec:analysis} is about more subtle and hard-to-apply forms of DPI.
...
a randomly generated subdomain of \urll{cloudflare.net}
...
the Fastly edge server returns an HTTP 400 error.
It is thus the only service for which the ``domainless'' fronting style is required.
...
The issue of statistical traffic anaysis is a general one~\cite{trafficmorphing},
...
specifically designed for meek.
...
The browser used was Tor Browser,
with the browser camouflage for meek described in Section~\ref{sec:browserextension},
using Google App Engine as the intermediate web service.
...
% anon
originating at Lawrence Berkeley National Laboratory (LBL).
...
The original IP addresses were replaced with a counter.
...
A few similarities are noticeable:
roughly 37\% of packets have no payload; these are mostly ACKs.
There is a small peak at the usual TCP Maximum Segment Size
of 1460 bytes.
% Other peaks near the max are probably near 1400, TLS maximum record size.
The meek trace is missing a small peak of small packets
present in the LBL trace.
In its place, there are minor peaks at fairly distinct values,
probably a reflection of Tor's fixed cell size
and the fixed overhead of an encrypted HTTP header added
to each transmission by meek.
% Maybe mention apparent residue of a doubling of the three peaks at around 750.
meek's trace is tends more overall to larger packets.
...
\caption{Daily unique users of meek with Psiphon3.}
\label{fig:clients}
\end{figure}
