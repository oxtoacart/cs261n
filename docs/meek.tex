...
Section~\ref{sec:discussion} describes more efficient encodings
...
As an encrypted tunneling protocol, meek adds some overhead.
Each chunk of data gets an HTTP header, and then the HTTP
request is wrapped in a TLS Application Data record.
...
but is somewhere in the neightborhood of $(543+450)/543-1\approx83$\%.
...
Bulk uploads and downloads tend to bunch many cells at once.
Persistent HTTP connections as used by Psiphon reduce overhead further,
because only one HTTP header is sent for a large number of transmissions.
...
There is additional latency overhead caused by the extra network hop:
rather than accessing the Tor bridge directly,
the client first goes through the intermediate web service before
reaching the bridge.
How much latency this adds, of course depends on the network location
...
because bandwidth becomes a function of latency.
...
Google's traffic is likely to be different than Akamai's.
...
Circumvention traffic must only be difficult enough to distinguish,
...
Most users who use Tor use Tor Browser~\cite{torbrowser},
...
Tor Browser has an easy interface for selecting a pluggable transport.
...
in June and scheduled for an alpha release,
...
which aggregates statistics self-reported by Tor relays.
...
estimated through clients' self-reporting of most recent connection date.
...
The experimental Tor Browser releases have all used Google with App Engine as the intermediate web service.
Support for CloudFront was not realized until late July.
We are running the backends on paid accounts for use by the public.
...
With client software can be configured to use the public backend by default,
there is nothing for the user to upload and no out-of-band communication is needed before getting
started---if everything works it offers no-configure circumvention.
...
high collateral damage of blocking the domain fronted through.
Fronting through \urll{www.google.com} to App Engine, for example,
is betting that censor will lack the temerity to block Google.
...
In 2012, the government of Pakisten blocked to all of YouTube
...
To block Google is an extreme measure,
probably out of reach of most censors.
...
or merely to get rid of their customers who are
engaged in fronting in the support of circumvention.
...
and the web service's specific motivations,
...
as a client's ISP would have if the client were connecting to Tor directly.
...
Even with Tor, in this case the web service gets to see
...
A possible mitigation is traffic obfuscation that lasts all
the way to the second hop of a three-hop Tor circuit,
...
If the edge server forwards the request to the Tor bridge
...
the costs and benefits before deciding to block SNI outright.

The current implementation of meek strictly serializes
its HTTPS requests and responses,
in order to keep the chunks of the underlying TCP stream
in the proper order.
The unfortunate side effect of this simple approach
is delay that gets worse with higher round-trip times.
...
has a standard list of trusted certificate authorities.
...
and detect fronting by decrypting HTTP requests and checking for specific Host headers.
...
ia man-in-the-middle attack merely to confirm that a user is circumventing,
...
We implemented meek,
a pluggable transport for Tor,
show that it resists common methods of blocking,
