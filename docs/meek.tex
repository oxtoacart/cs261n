...
the censor,
the censored client,
the intermediate web service's frontend server (CDN edge server),
and the destination proxy (Tor bridge).
Circumvention is achieved when the client evades the censor to reach
the destination proxy,
because the proxy provides access to anywhere.
The client and destination proxy cooperate with each other.
...
while the intermediate web service and destination proxy lie outside the network.
The censor blocks direct communication between the client and the destination proxy,
...
See Section~\ref{sec:security} for what happens when some
...
% Use a child's garden--like figure of the HTTP protocol here.
% "concatenates successive bodies"
