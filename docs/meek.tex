...
the existence of meek traffic according to the \emph{metadata}, i.e. traffic characteristics. 
...
the same Google frontend server, the browser will keep the persistant TLS connection. In addition, small requests 
...
These browser techniques improve performance, but also lead to unexpected results. The censor may be aware of the 
...
Missing parts of the graph are caused by missing data at the metrics portal.
}
\label{fig:clients}
\end{figure}
