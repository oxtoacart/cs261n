...
\strut~~~~--header 'Host: maps.google.com' | \char`\\\\
\strut~~~~grep -o '<title>.*</title>'\\
...
% See Section~\ref{sec:deployment} for more discussion on deployment.
...
Fronting has been used with success since 2013~\cite{flashproxy-reg-appspot}
for the rendezvous component of another transport called flash proxy.
...
To the above challenges we may add the related challenge of active probing.
An active-probing censor tries to connect to a server using a circumvention tool
as if it were an ordinary client.
If the connection is successful, the server can be blacklisted.
Active probing is a very precise means of identifying a server as belonging to a circumvention system.
...
Classic examples of look-like-nothing
...
shapes statistical features such as packet length and byte frequency to
...
translate a web page at \urll{http://proxy.example.com/...},
where ``\urll{...}''
...
In decoy routing, friendly ISPs deploy special routers that lie
...
% GoAgent not working since June 2014.
...
We assume that the censor cannot observe the communication
once it reaches the third-party infrastructure; i.e.,
that no further obfuscation is needed once fronting is accomplished.
